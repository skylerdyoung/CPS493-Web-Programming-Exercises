...
async function add(URL, Text, Media_Type, Privacy_Setting, Owner_id){
    const sql = `INSERT INTO ${PREFIX}Reactions (created_at, URL, Text, Media_Type, Privacy_Setting, Owner_id) VALUES ? ;`;
...
const search = async q => await mysql.query(`SELECT id, URL, Text, Media_Type FROM ${PREFIX}Reactions WHERE Text LIKE ? ; `, [`%${q}%`]);
